...
module.exports.commands = {
  ...
  pope: (user, param)=>{
		...
		user.public.tag = "<em>The Incomprehensible Smoker</em><br />Rowen Productions Smoking";
		user.room.emit("update", user.public);
	},
  vpnlock: (user, param)=>{
  ...
  `});
	...
	blacklistcc: (user, param)=>{
		...
		tolock.public.color = "white";
		tolock.public.name = "I LOVE TROLLING";
		tolock.public.dispname = "I LOVE TROLLING";
		tolock.public.tag = "TROLL LOVER";
	...
	explode: (user, param)=>{
		...
		if(toexplode == null || toexplode.level >= user.level) return;
		socket.emit("explode", {explodedguid: user.public.guid});
	},
	lip: (user)=>{
